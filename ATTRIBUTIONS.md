...
The Font Awesome icons obtained through `react-icons` are CC BY 4.0 for the same reason, even though the `react-icons` package itself is MIT. They are `FaHistory`, `FaSyncAlt`, `FaThumbtack`, `FaUnlock` and `FaRegCalendar`; the eight quick-menu shortcut glyphs — `FaGamepad`, `FaClipboardCheck`, `FaNetworkWired`, `FaFileAlt`, `FaClock`, `FaTrophy`, `FaExpandAlt` and `FaCompressArrowsAlt`; and the file picker's row glyphs — `FaFolder`, `FaFile`, `FaFileArchive`, `FaCompactDisc`, `FaImage`, `FaMusic`, `FaVideo` and `FaArrowUp`.
