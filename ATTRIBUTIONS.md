...
The Font Awesome icons obtained through `react-icons` are CC BY 4.0 for the same reason, even though the `react-icons` package itself is MIT. They are `FaHistory`, `FaSyncAlt`, `FaThumbtack`, `FaUnlock`, `FaRegCalendar` and `FaRegQuestionCircle`; the ten quick-menu shortcut glyphs — `FaGamepad`, `FaClipboardCheck`, `FaNetworkWired`, `FaFileAlt`, `FaClock`, `FaCalculator`, `FaTrophy`, `FaExpandAlt`, `FaCompressArrowsAlt` and `FaExchangeAlt`; and the file picker's row glyphs — `FaFolder`, `FaFile`, `FaFileArchive`, `FaCompactDisc`, `FaImage`, `FaMusic`, `FaVideo` and `FaArrowUp`; and the browser's controls — `FaArrowLeft`, `FaArrowRight`, `FaBars`, `FaCheck`, `FaCheckCircle`, `FaChevronDown`, `FaChevronLeft`, `FaChevronRight`, `FaChevronUp`, `FaFolderPlus`, `FaGlobe`, `FaMinus`, `FaPen`, `FaPlus`, `FaRegCircle`, `FaRegStar`, `FaSearch`, `FaStar`, `FaStop`, `FaTimes`, `FaTrash`, `FaWindowMaximize` and `FaWindowRestore`.
